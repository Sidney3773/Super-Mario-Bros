function createHUD() {
    ...
    if (localHighScore !== null) {
        this.highScoreText.setText('MEJOR PUNTAJE\n' + localHighScore.toString().padStart(6, '0'))
    }
...
function gameOverScreen(outOfTime=false) {
    ...
    this.add.bitmapText(screenCenterX, screenHeight / 3, 'carrier_command', outOfTime ? 'TIEMPO AGOTADO' : 'JUEGO TERMINADO', screenWidth / 30).setOrigin(0.5).depth = 5;
    ...
    this.add.bitmapText(screenCenterX, screenHeight / 1.7, 'carrier_command', '> TOMAR FOTO', screenWidth / 50).setOrigin(0.5).setInteractive().on('pointerdown', () => getScreenshot()).depth = 5;
}
...
function winScreen() {
    ...
    this.add.bitmapText(screenCenterX, screenHeight / 3, 'carrier_command', '¡GANASTE!', screenWidth / 30).setOrigin(0.5).depth = 5;
    this.add.bitmapText(screenCenterX, screenHeight / 2, 'carrier_command', '> JUGAR DE NUEVO', screenWidth / 50).setOrigin(0.5).setInteractive().on('pointerdown', () => location.reload()).depth = 5;
    this.add.bitmapText(screenCenterX, screenHeight / 1.7, 'carrier_command', '> TOMAR FOTO', screenWidth / 50).setOrigin(0.5).setInteractive().on('pointerdown', () => getScreenshot()).depth = 5;
}
